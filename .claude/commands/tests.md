---
description: Run week4 pytest suite with optional path/marker; report coverage on success
...
## Steps
1. `cd week4` (or confirm cwd is `week4/`).
...
4. If tests **pass**:
   - Run coverage (install if missing: `pip install pytest-cov`):
     ```bash
...
## Safety
- Only run pytest/black/ruff inside `week4/`.
- Do not modify production DB files under `week4/data/` except via test fixtures.
